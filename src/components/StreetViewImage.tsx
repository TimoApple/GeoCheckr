// GeoCheckr — Street View v10
// PROVEN approach: Metadata API → exact coords → Static API → Image
// No WebView. No consent. No block. Just works.
...
export default function StreetViewImage({ location, showInfo = false }: StreetViewProps) {
  ...
  useEffect(() => {
    ...
    const findStreetView = async () => {
      try {
        // Step 1: Metadata API — find nearest panorama (exact coords)
        ...
        if (meta.status === 'OK' && meta.location) {
          // Step 2: Static API with EXACT coordinates from metadata
          ...
          console.log('[GeoCheckr] Street View:', meta.location.description || location.city);
        } else {
          console.log('[GeoCheckr] No coverage:', location.city, meta.status);
          setStatus('fallback');
        }
      } catch (err) {
        console.warn('[GeoCheckr] Error:', err);
        setStatus('fallback');
      }
    };
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Static Street View — real image from Google */}
      {status === 'streetview' && staticUrl && (
        <Image
          ...
          resizeMode="cover"
          onError={() => {
            console.warn('[GeoCheckr] Image load failed');
            setStatus('fallback');
          }}
        />
      )}
