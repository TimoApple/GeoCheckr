// GeoCheckr v7 — Static Image ONLY (guaranteed to work)
...
export default function App() {
  ...
  const startGame = () => {
    ...
    popAnim.setValue(0);
    const loc = o[0];
    setCurrentLoc(loc); setTimer(30); setHeading(Math.floor(Math.random()*360));
  ...
  const nextRound = () => {
    ...
    setCurrentLoc(next); setTimer(30); setRound(r => r + 1); setHeading(Math.floor(Math.random()*360));
    setScreen('streetview');
  };
  ...
  if(screen==='home') return (
    <View style={s.container}>
      ...
      <View style={s.homeContent}>
        ...
        <TouchableOpacity style={s.modeBtn} onPress={startGame}>
          <Text style={s.modeBtnIcon}>📸</Text>
          <Text style={s.modeBtnLabel}>SPIELEN</Text>
          <Text style={s.modeBtnSub}>10 Runden • 10 Städte</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
  ...
  if(screen==='result' && result) return (
    <View style={s.container}>
      <StatusBar barStyle="light-content" />
      <View style={s.bgOrbs}>
        <View style={[s.orb,{top:'20%',left:'20%',width:200,height:200,backgroundColor:result.correct?'#1a4a1a':'#4a1a1a'}]}/>
      </View>
